function  gretna_plot_regression_spec2(Xdata, Ydata, NumDegree, Type_hist)
...
switch lower(Type_hist)
    ...
    case 'off'
        plot(Xdata, Ydata, 'm.' ,'Markersize', 8);
    otherwise
...
% plot fit curve
plot(Xdata, yfit, 'color', [0.75 0.45 0.15], 'Linewidth', 2);

% Add title and legend 
%title(['{\it Fit: }','\it',gretna_polystr(round(100*p)/100)],'FontSize',12,...
%    'FontName','Times New Roman');
%legend('Subject', '95% CI', 'Fitted curve','Location','best');
%legend('boxoff');
